...
if [ ! -f "$BOOTSTRAP_JAR" ]; then
    ...
    if ! curl -fL -o "$BOOTSTRAP_JAR" \
        "https://github.com/packwiz/packwiz-installer-bootstrap/releases/download/v0.0.3/$BOOTSTRAP_JAR"; 
    then
...
if [ ! -f "$INSTALLER_JAR" ]; then
    ...
    if ! curl -fL -o "$INSTALLER_JAR" \
        "https://github.com/packwiz/packwiz-installer/releases/download/v0.5.14/$INSTALLER_JAR";
    then
        echo "下载失败，请检查网络连接或证书配置"
        exit 3
    fi
